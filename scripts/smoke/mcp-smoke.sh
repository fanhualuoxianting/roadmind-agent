...
if [[ -z "${ROADMIND_MCP_TOKEN:-}" || "$ROADMIND_MCP_TOKEN" == "disabled" ]]; then
    echo 'ROADMIND_MCP_TOKEN must be supplied for MCP smoke; no token is hard-coded.' >&2
    exit 2
fi
...
mcp_request() {
    ...
    body="${raw%$'\n'*}"
    if [[ "$status" != "200" ]]; then
        echo "MCP request returned HTTP $status" >&2
        exit 1
    fi
    printf '%s' "$body"
}
...
validate_mcp "$initialize" 'document.get("result", {}).get("serverInfo", {}).get("name") == "roadmind-controlled-tools"' 'initialize server info'

tools="$(mcp_request '{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}')"
validate_mcp "$tools" 'set(tool.get("name") for tool in document.get("result", {}).get("tools", [])) >= {"vehicle.get_status", "weather.get_forecast", "route.plan"}' 'read-only tool catalog'
...
validate_mcp "$call" '"error" not in document and document.get("result", {}).get("isError") is not True' 'safe vehicle tool call'
